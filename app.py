...
def get_graphs(date):
    # Read raw csv file from GitHub repo and save into dataframe. User specifies date for daily report.
    ...
    # Gather daily statistics including total confirmed cases, total death cases, mean confirmed cases, mean death cases and ratio between total confirmed and total death cases.
    ...
    figScatter = px.scatter(world, x="Confirmed", y="Deaths", hover_data=["Country"], color="Confirmed", title="Interactive Scatter Map w/ Confirmed and Death Cases", template="plotly_dark")
    ...
    pieConfirmed = px.pie(top_20, values='Confirmed', names='Country', template="plotly_dark")
    ...
    pieDeath = px.pie(top_20, values='Deaths', names='Country', template="plotly_dark")
...
def update_output(date_value):
    ...
    if date_value is not None:
        ...
        # Check if file is available, can get 404 error so return data is not available. Graphs are empty if file is not available.
        try:
        ...
        scatterPlot, mapPlot, barConfirmed, barDeaths, pieConfirmed, pieDeath, graphStatistics= get_graphs(date_string)
